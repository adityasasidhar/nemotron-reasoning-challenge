...
WARMUP_FRAC = 0.03
LR_FLOOR_FRAC = 0.0
GRAD_CLIP_NORM = 1.0          # was 1e9 — weighted loss makes gradients spikier
EPOCHS = 3
BRANCH_LOGPROB = 0.01         # huikang's cutoff: tokens with NLL below this get down-weighted
USE_LOGPROB_WEIGHTING = True  # False = plain multi-epoch+shuffle baseline (Run A)
RESET_WEIGHTS = True
IN_PROJ_ONLY = False
MOE_TIE_WEIGHTS = True
ORIGINAL_PROBLEMS_ONLY = False
SHUFFLE_DATASET = False
USE_MATH_REPLAY = False
...
def run_training() -> None:
    ...
    # Full training schedule — keep duplicates AND all epochs.
    # The repetition in this file IS huikang's difficulty curriculum.
    ordered_ids: list[str] = []
    with open(TRAIN_ORDER_PATH) as f:
        for line in f:
            ordered_ids.append(json.loads(line)["problem_id"])

    print(f"Schedule: {len(ordered_ids)} instances, {len(set(ordered_ids))} unique")

    correct_set = None
    if CORRECT_ONLY:
        with open("/kaggle/input/datasets/tadityasasidhar/correct-ids/correct_ids.json") as f:
            correct_set = set(json.load(f))
    ...
    N = len(examples)
    batches_per_epoch = (N + BATCH_SIZE - 1) // BATCH_SIZE   # ceil → every example seen each epoch
    num_steps = batches_per_epoch * EPOCHS
    ref_nll: list = [None] * N      # per-example, per-token NLL cache (fp16); ~56 MB total
    ...
    _log(f"Training: {EPOCHS} epochs x {batches_per_epoch} = {num_steps} steps, batch={BATCH_SIZE}, "
         f"micro={MICRO_BATCH_SIZE}, lr={LEARNING_RATE}, schedule={LR_SCHEDULE}, clip={GRAD_CLIP_NORM}, "
         f"weighting={USE_LOGPROB_WEIGHTING}, branch_logprob={BRANCH_LOGPROB}")
    ...
    for epoch in range(EPOCHS):
        order = list(range(N))
        rng.shuffle(order)                       # per-epoch shuffle — kills fixed-order/LR coupling
        _log(f"=== epoch {epoch}/{EPOCHS} (shuffled {N} examples) ===")
        ...
        for batch_start in range(0, N, BATCH_SIZE):
            ...
            for mb_start in range(0, n, MICRO_BATCH_SIZE):
                ...
                mb_global = batch_idx[mb_start:mb_end]        # global indices for the cache
                ...
                use_weighting = USE_LOGPROB_WEIGHTING and epoch > 0
                if use_weighting:
                    diff_mult = torch.zeros(n_micro, max_len, dtype=torch.float32, device=device)
                    for i in range(n_micro):
                        seq_len = len(mb_toks[i])
                        nll = ref_nll[mb_global[i]]
                        if nll is not None:
                            diff_mult[i, :seq_len] = (nll.to(device).float() / BRANCH_LOGPROB).clamp(max=1.0)
                    eff_weights = padded_weights * diff_mult
                else:
                    eff_weights = padded_weights
                ...
                with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                    ...
                    # Refresh the difficulty cache every epoch (uses latest logprobs, like huikang's prev_logprobs)
                    if USE_LOGPROB_WEIGHTING:
                        with torch.no_grad():
                            for i in range(n_micro):
                                seq_len = len(mb_toks[i])
                                ref_nll[mb_global[i]] = per_token_ce[i, :seq_len].detach().to(torch.float16)

                    weighted_loss = per_token_ce * eff_weights
                    weight_sum_t = eff_weights.sum()
                ...
                if use_weighting and not _printed_diag:
                    with torch.no_grad():
                        m = padded_weights > 0
                        if m.any():
                            dm = diff_mult[m]
                            _log(f"  [diag] answer-token diff_mult: mean={dm.mean():.3f} "
                                 f"frac<0.5={(dm < 0.5).float().mean():.3f} "
                                 f"frac==1={(dm >= 1.0).float().mean():.3f}")
                            _printed_diag = True
            ...
            step += 1
            _log(f"  step {step}/{num_steps} (epoch {epoch}): loss={loss_mean:.6f}, "
